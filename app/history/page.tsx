...
export default function History() {
    ...
    void (async () => {
      try {
        ...
        const { data, error } = await s
          .from('reports')
          .select('id,title,report_date,location,description,urgency,recommended_action,status,approved_at')
          .eq('user_id', user.id)
  ...
  return (
    <>
      <div className="top">
        <div className="brand">Historie</div>
        <div className="badge">{rows.length} Berichte</div>
      </div>
      ...
      <div className="grid">
        {rows.map((r) => (
          <div className="card" key={r.id}>
            ...
            <span>
              {r.report_date} · {r.location || 'ohne Ort'} · {r.status}
            </span>
            <div style={{ height: 10 }} />
